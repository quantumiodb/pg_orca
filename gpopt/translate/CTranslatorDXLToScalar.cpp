...
/* PG18 compat: AGGSPLIT_INTERMEDIATE removed — map to AGGSPLIT_INITIAL_SERIAL */
	...
	const CDXLNode *scalar_winref_node, CMappingColIdVar *colid_var)
{
	...
	/* windistinct was removed from WindowFunc in PG18 — ignored */
	(void) dxlop->IsDistinct();
	...
	Plan *plan, SubLinkType slink, CContextDXLToPlStmt *dxl_to_plstmt_ctxt)
{
	...
	/* is_initplan removed from PG18's SubPlan */
	...
	/* is_multirow removed from PG18's SubPlan */
...
)
{
	...
	/* DMLActionExpr is GPDB-only; in PG18 single-node mode return a null Const */
	Const *expr = makeNode(Const);
	expr->consttype = INT4OID;
	expr->consttypmod = -1;
	expr->constcollid = InvalidOid;
	expr->constlen = sizeof(int32);
	expr->constvalue = Int32GetDatum(0);
	expr->constisnull = false;
	expr->constbyval = true;
	expr->location = -1;
