...
/* cdb/cdbhash.h not in PG18 — cdb hash functions stubbed in gpdbwrappers */
...
#endif
/* GPDB-specific AM OIDs — not in PG18 */
#ifndef PAX_AM_OID
#define PAX_AM_OID 65536
#endif
#ifndef AO_COLUMN_TABLE_AM_OID
#define AO_COLUMN_TABLE_AM_OID 65537
#endif
#ifndef AO_ROW_TABLE_AM_OID
#define AO_ROW_TABLE_AM_OID 65538
#endif
/* GPDB-specific relkind */
#ifndef RELKIND_AOSEGMENTS
#define RELKIND_AOSEGMENTS 's'
#endif
}
									  ...
									  IMDId *mdid)
{
	...
	/* rd_cdbpolicy is GPDB-only; in PG18 single-node mode all tables are ENTRY */
...
CTranslatorRelcacheToDXL::GetDistributionFromForeignRelExecLocation(
	ForeignTable *ft)
{
	/* exec_location / FTEXECLOCATION_* are GPDB MPP-only.
	 * In PG18 single-node mode, foreign tables are treated as universal. */
	(void) ft;
...
CTranslatorRelcacheToDXL::RetrieveType(CMemoryPool *mp, IMDId *mdid)
{
	...
	CMDIdGPDB *mdid_count =
		/* F_COUNT_ANY is GPDB-specific; look up count(any) at runtime */
		GPOS_NEW(mp) CMDIdGPDB(IMDId::EmdidGeneral, gpdb::GetAggregate("count", ANYOID));
