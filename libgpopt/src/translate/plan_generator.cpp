...
static bool SetHashKeysVarnoWalker(Node *node, void *context) {
  ...
  return expression_tree_walker(node, (bool (*)()) SetHashKeysVarnoWalker, context);
}
...
Plan *PlanGenerator::GenerateSeqScanPlan(PlanGeneratorContext *ctx) {
  ...
  auto *plan = &seq_scan->plan;
  plan->plan_node_id = GetNextPlanNodeID();
  plan->targetlist = GeneratePlanTargetList(seq_scan->scanrelid, cols, ctx->out_cols, true);
